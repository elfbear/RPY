...
int main(int argc, char **argv)
{
  ...
  fp0= fopen("loc.xls","w");
  for(j=0; j<nparts; j++){
  	fprintf(fp0, "%f\t %f\t %f\n", ploc[3*j], ploc[3*j+1], ploc[3*j+2]);
  }
  fclose(fp0);

  //call RPYfmm to calculate RPY
  RPYfmm(beta, s, accuracy, nparts, ploc, pcharge, RPY);
...
void test_data(int nparts, int distribution, double *ploc, double *pcharge)
{
  if ( distribution == 1 ) {
    ...
    for ( i = 0; i < nparts; i++ ) {
	  ...
	  pcharge[j+1] =1.0;
	  pcharge[j+2] =1.0;
      ploc[j] = ((double) rand()/RAND_MAX - 0.5)*1;
      ploc[j+1] = ((double) rand()/RAND_MAX - 0.5)*1;
      ploc[j+2] = ((double) rand()/RAND_MAX - 0.5)*1;
    }
